...
Configuration is read on demand from server environment variables, `.env`, then `.env.example` (in descending precedence). This supports editing the local key without restarting Python; reopening chat refreshes the configured status. Only `configured` and model name leave `/api/chat/status`, never credentials. Follow the [OpenAI quickstart](https://developers.openai.com/api/docs/quickstart) for server-side API-key setup. Commit only the blank example. If a real key ever enters Git history, rotate it and exclude that history from publication; changing the current file is not enough.
